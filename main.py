"""CLI entry point for the WeChat article scraper."""
...
@click.option(
    ...
    default=None,
    type=int,
    help="最多爬取搜索结果页数（覆盖 config.yaml 中的 max_pages）",
)
@click.option(
    "--output-dir", "-o",
    default=None,
...
def main(
    ...
    \b
        ...
        python main.py 人工智能 --no-content --headful
    """
    ...
    headless = not headful and scraper_cfg.get("headless", True)

    console.rule(f"[bold]微信公众号文章爬虫[/bold]")
...
async def _run(
    ...
    if not articles:
        console.print("[bold red]未抓取到任何文章，请检查关键词或网络环境。[/bold red]")
        return
